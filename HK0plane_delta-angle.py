...
Delta_omega =np.max(omega)-np.min(omega)
"measure_omega=",np.max(omega)+5," - ",np.min(omega)-5," = ",np.max(omega)-np.min(omega)+10
#print
...
d1=pd.DataFrame([omega,theta*2,tau],index=["omega","2theta","tau"], columns=['(1,0,0)','(1+q,0,0)','(1-q,0,0)','(1,q,0)','(1,-q,0)','(1+q,-q,0)','(1-q,q,0)'])
